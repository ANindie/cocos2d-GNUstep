...
static NSString *transitions[] = {
				...
				@"SpriteEaseBackInOut",
				@"SpeedTest",
				/*@"SchedulerTest",*///NSSlider is not working 
};
...
-(id) init
{
	if( (self=[super init])) {
		...
		CCTexture2D *tex = [[CCTexture2D alloc] initWithImage: [[UIImage alloc] initWithContentsOfFile: [[[CCConfiguration sharedConfiguration] loadingBundle] pathForResource:@"grossini.png" ofType:nil] ] ];
...
-(void) positionForTwo
{	
	[grossini setPosition: ccp( 60, 120 )];
	[tamara setPosition:ccp( 60, 220)];
...
- (NSSlider *)sliderCtl
{
    if (sliderCtl == nil) 
    {
        ...
        sliderCtl = [[NSSlider alloc] initWithFrame:frame];
        ...
        //[sliderCtl setBackgroundColor: [NSColor clearColor]];
        
        [sliderCtl setMinValue:0.0f];
        [sliderCtl setMaxValue: 2.0f];
        [sliderCtl setContinuous: YES];
        [sliderCtl setDoubleValue: 1.0f];
		
		[sliderCtl setTag: kTagSlider];	// tag this view for later so we can remove it from recycled table cells
    }
...
-(void) sliderAction:(id) sender
{
	...
	[[CCScheduler sharedScheduler] setTimeScale: [sliderCtl doubleValue]];
}
...
-(void) onEnter
{
	...
	[[[[[CCDirector sharedDirector] openGLView] window] contentView] addSubview: sliderCtl];
}
