...
DefineProperty_rw_as_na(CGPoint,velocity,Velocity,velocity);
...
{
	[self setPosition : ccpAdd([self position], ccpMult(velocity, delta))];
...
- (void)collideWithPaddle:(Paddle *)paddle
{
	...
	if ([self position].x > leftX && [self position].x < rightX) {
		...
		if ([self position].y > midY && [self position].y <= highY + [self radius]) {
			[self setPosition : CGPointMake([self position].x, highY + [self radius])];
			hit = YES;
			angleOffset = (float)M_PI / 2;
		}

		else if ([self position].y < midY && [self position].y >= lowY - [self radius]) {
			[self setPosition : CGPointMake([self position].x, lowY - [self radius])];
			hit = YES;
			angleOffset = -(float)M_PI / 2;
		}
